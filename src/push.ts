...
export async function push(
	sourcePath: string,
	hackmudPath: string,
	{ scripts = [ `*.*` ], onPush = () => {}, minify = true, mangleNames = false, forceQuineCheats, rootFolderPath }: PushOptions = {}
	...
	const allUsers = new Set([
		...
		...hackmudFolder.filter(({ stats, name }) => stats.isFile() && name.endsWith(`.key`))
